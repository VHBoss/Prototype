...
public class IndentDraw : MonoBehaviour
{
    ...
    public void IndentAt(RaycastHit hit, float scale = 1)
    {
        ...
        DrawAt(x * targetTexture.width, y * targetTexture.height, 1.0f, scale);
    }
    ...
    void DrawAt(float x, float y, float alpha, float scale)
    {
        ...
        stampWidth = stampTexture.width * scale;
        stampHeight = stampTexture.height * scale;

        // put the center of the stamp at the actual draw position
        screenRect.x = x - stampWidth * 0.5f;
        screenRect.y = (targetTexture.height - y) - stampHeight * 0.5f;
        screenRect.width = stampWidth;
        screenRect.height = stampHeight;
